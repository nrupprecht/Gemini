...
void Image::Impl::Relation_Fix(
    ...
    CanvasPart canvas2_part,
    double pixels_diff) {
  fixes_.push_back(std::make_shared<FixRelationship>(canvas1, canvas2, canvas1_part, canvas2_part, pixels_diff));
}
...
void Image::Impl::Scale_Fix(
    ...
    CanvasDimension dimension,
    double lambda) {
  fixes_.push_back(std::make_shared<FixScale>(canvas1, canvas2, canvas1_part, dimension, lambda));
}
...
void Image::Impl::Dimensions_Fix(
    ...
    CanvasDimension dim,
    double extent) {
  fixes_.push_back(std::make_shared<FixDimensions>(canvas, dim, extent));
}
...
void Image::Impl::RelativeSize_Fix(
    ...
    CanvasDimension dimension2,
    double scale) {
  fixes_.push_back(std::make_shared<FixRelativeSize>(canvas1, canvas2, dimension1, dimension2, scale));
}
